...
vec4 fxaa(sampler2D tex, vec2 uv, vec2 rcpFrame) {
    ...
    vec3 rgbNW = texture2D(tex, uv + vec2(-1.0, -1.0) * rcpFrame).xyz;
    vec3 rgbNE = texture2D(tex, uv + vec2(1.0, -1.0) * rcpFrame).xyz;
    vec3 rgbSW = texture2D(tex, uv + vec2(-1.0, 1.0) * rcpFrame).xyz;
    vec3 rgbSE = texture2D(tex, uv + vec2(1.0, 1.0) * rcpFrame).xyz;
...
void main() {
	...
	if (sunvisibility > 0.01) {
        ...
        // Add FXAA
        vec2 rcpFrame = vec2(1.0 / viewSize.x, 1.0 / viewSize.y);
        vec4 scene = vec4(c, 1.0);
        // gaux1
        scene = fxaa(gaux1, newTC, rcpFrame);
        // gcolor
        // scene = fxaa(gcolor, newTC, rcpFrame);
        ...
        c = scene.rgb * (0.8+vignette*1.2)*0.5;
        #endif  
